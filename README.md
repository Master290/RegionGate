...
`REGIONGATE_MAX_PACKET_SIZE`, `REGIONGATE_QUEUE_SIZE`, and
`REGIONGATE_ADMISSION_INTERVAL`.
Login throttling is configured with `REGIONGATE_LOGIN_RATE_LIMIT` and
`REGIONGATE_LOGIN_RATE_WINDOW`.

Session limits can be tuned with `REGIONGATE_HANDSHAKE_TIMEOUT`,
`REGIONGATE_STATUS_TIMEOUT`, `REGIONGATE_LOGIN_TIMEOUT`,
`REGIONGATE_WRITE_TIMEOUT`, `REGIONGATE_KEEPALIVE_INTERVAL`,
`REGIONGATE_KEEPALIVE_TIMEOUT`, `REGIONGATE_QUEUE_STATUS_INTERVAL`, and
`REGIONGATE_CHALLENGE_TIMEOUT`. Online-mode clients use
`REGIONGATE_SESSION_SERVER_URL` to override Mojang's session endpoint in
